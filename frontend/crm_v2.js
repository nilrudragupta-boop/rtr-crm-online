const V2 = (() => {
    ...
    async function search(term) { if (!term.trim()) { document.getElementById('results').style.display = 'none'; return } try { const rows = await get('/crm/v2/search?q=' + encodeURIComponent(term)); const el = document.getElementById('results'); el.innerHTML = rows.length ? rows.map(x => `<div class="result" data-type="${q(x.type)}" data-id="${q(x.id)}" data-ref="${q(x.ref || x.id)}"><b>${q(x.type)}</b> · ${q(x.ref)}<div style="font-size:11px;color:#667085">${q(x.name)}</div></div>`).join('') : '<div class="result">No records found.</div>'; el.style.display = 'block' } catch (e) { console.warn(e) } }
    function bind() { const input = document.getElementById('globalSearch'), results = document.getElementById('results'); let timer; input.addEventListener('input', () => { clearTimeout(timer); timer = setTimeout(() => search(input.value), 220) }); document.addEventListener('click', e => { const r = e.target.closest('.result'); if (r && r.dataset.id) { const type = r.dataset.type.toLowerCase(); const value = r.dataset.ref || r.dataset.id; const page = { customer: 'business360.html', supplier: 'business360.html', enquiry: 'enquiry.html', quotation: 'quotation.html', invoice: 'invoice.html' }[type]; if (type === 'customer' || type === 'supplier') location.href = `${page}?type=${type}&id=${encodeURIComponent(r.dataset.id)}`; else if (page) location.href = page + '?' + (type === 'quotation' ? 'refNo=' : 'id=') + encodeURIComponent(value); return } if (!e.target.closest('.search')) results.style.display = 'none' }); document.getElementById('modules').innerHTML = [['Customers', 'customer.html', 'Customer records'], ['Suppliers', 'supplier.html', 'Supplier records'], ['Enquiries', 'enquiry.html', 'Sales intake'], ['Quotations', 'quotation.html', 'Commercial'], ['Customer 360°', 'business360.html', 'Business view'], ['Follow-ups', 'follow_up.html', 'Action items'], ['Customer support', 'customer_support.html', 'Service work'], ['Purchase', 'purchase.html', 'Buying work'], ['Invoices', 'invoice.html', 'Billing'], ['Reports', 'report.html', 'Management'], ['Order execution', 'order-sheet.html', 'Operations'], ['Purchase dashboard', 'purchase_dashboard.html', 'Procurement'], ['Invoice dashboard', 'invoice_dashboard.html', 'Accounts']].map(x => `<div class="module" onclick="location.href='${x[1]}'"><b>${x[0]}</b><span>${x[2]}</span></div>`).join('') }
