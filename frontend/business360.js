    ...
    renderOverview(vp) {
        ...
        vp.innerHTML = `${(d.alerts || []).map(a => `<div class="b360-alert-banner b360-alert-${this.esc(a.type)}">${a.icon} ${this.esc(a.text)}</div>`).join('')}
            <div class="b360-overview-grid">
                ...
                <div class="slds-card"><div class="slds-card-header">Key Contacts [Data from 'Contacts' page' only](${d.contacts?.length || 0})</div><div class="slds-card-body">${this.contactsHtml, supplierHTML(d.contacts)}</div></div>
            </div>
