...
const Chatter = {
    ...
    renderGroups: function() {
        ...
        groupList.innerHTML += `<div class="group-item ${globalActive}" onclick="Chatter.switchGroup('global')"><span><i class="fas fa-globe text-primary me-2"></i> Global Coordination</span>${globalUnread}</div>`;
    ...
    switchGroup: function(groupId) {
        ...
        if(titleEl) {
            if (groupId === 'global') {
                titleEl.innerHTML = `<i class="fas fa-globe me-1"></i> Global Coordination`;
    ...
    renderPinnedMessage: function() {
        ...
        if (pinnedMsg) {
            ...
            banner.onclick = () => document.getElementById('msg-' + pinnedMsg.id)?.scrollIntoView({behavior: 'smooth', block: 'center'});
        } else {
