...
// --- WEB BROWSER POLYFILL FOR ELECTRON API ---
// This seamlessly translates desktop IPC calls to Web LocalStorage when hosted on Render
if (typeof window !== 'undefined' && !window.electronAPI) {
    console.log("🌐 Web Browser detected. Initializing ElectronAPI Polyfill...");
    window.electronAPI = {
        getAppSettings: async () => {
            const s = JSON.parse(localStorage.getItem('web_app_settings')) || {};
            return {
                success: true,
        ...
        saveAppSettings: async (params) => {
            const updates = JSON.parse(localStorage.getItem('web_app_settings')) || {};
            Object.keys(params).forEach(key => {
                const upperKey = key.replace(/([A-Z])/g, '_$1').toUpperCase();
                if (params[key] !== undefined) updates[upperKey] = params[key];
            });
            localStorage.setItem('web_app_settings', JSON.stringify(updates));
            return { success: true };
        },
        getAdminCreds: async () => {
            try {
                // 1. Fetch live credentials from MongoDB
                const res = await fetch('/api/admin-creds');
                const data = await res.json();
                if (data.success) {
                    // Cache locally for offline fallback
                    localStorage.setItem('web_admin_creds', JSON.stringify(data));
                    return data;
                }
            } catch (e) {
                console.warn("MongoDB fetch failed, falling back to local storage.", e);
            }

            // 2. Offline Fallback
            const s = JSON.parse(localStorage.getItem('web_admin_creds')) || {};
            return {
                success: true,
                adminUser: s.adminUser || s.ADMIN_USERNAME || "Admin",
                adminEmail: s.adminEmail || s.ADMIN_EMAIL || "",
                adminEmailPass: s.adminEmailPass || s.ADMIN_EMAIL_PASS || "",
                adminPass: s.adminPass || s.ADMIN_LOGIN_PASS || "",
                adminUsers: s.adminUsers || s.ADMIN_USERS || [{ username: "Admin", password: "", role: "Admin" }]
            };
        },
        saveAdminCreds: async (params) => {
            try {
                // 1. Save to MongoDB
                await fetch('/api/admin-creds', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(params) });
            } catch (e) { console.error("Failed to save creds to MongoDB", e); }

            // 2. Save locally
            const updates = JSON.parse(localStorage.getItem('web_admin_creds')) || {};
            if (params.adminUser !== undefined) updates.ADMIN_USERNAME = params.adminUser;
            if (params.adminEmail !== undefined) updates.ADMIN_EMAIL = params.adminEmail;
            if (params.adminEmailPass !== undefined) updates.ADMIN_EMAIL_PASS = params.adminEmailPass;
            if (params.adminPass !== undefined) updates.ADMIN_LOGIN_PASS = params.adminPass;
            if (params.adminUsers !== undefined) updates.ADMIN_USERS = params.adminUsers;
            localStorage.setItem('web_admin_creds', JSON.stringify(updates));
            return { success: true };
        },
        getSystemInfo: async () => ({ success: true, os: navigator.userAgent, cpu: "Web Browser", memory: "N/A", display: "N/A" }),
        getAppVersion: async () => "Web Version 1.0",
        checkRemoteLicense: async () => ({ success: true }), // Bypass strict desktop license check on the web
        checkForUpdates: async () => ({ success: true, updateAvailable: false }),
        dbRead: async (key) => { const data = localStorage.getItem('web_db_' + key); return data ? JSON.parse(data) : null; },
        dbWrite: async (key, value) => { localStorage.setItem('web_db_' + key, JSON.stringify(value)); return { success: true }; },
